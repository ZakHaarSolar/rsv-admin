// admin/supabase/functions/upload-avatar/index.ts v1.0
// v1.0 — Foto de perfil del Tripulante → Cloudflare R2. Resuelve el bug de la
//        "Firma Visual" en iOS nativo: la subida de Clerk (setProfileImage) va
//        como multipart a la FAPI de Clerk y NO funciona en el WKWebView
//        (CapacitorHttp no arma multipart; el fetch nativo manda Origin inválido
//        → Clerk rechaza). En vez de eso subimos la imagen a R2 (por Supabase,
//        sin el chequeo de Origin de Clerk) y el cliente guarda la URL en
//        unsafeMetadata.avatarUrl (un update JSON que SÍ pasa por el puente).
//
...
/* AUDITORÍA PARTE 3 · el tipo real se deduce de los bytes y el peso se corta
   antes de decodificar (el bucket es público: un HTML declarado "image/png"
   quedaba servido bajo nuestro dominio, y no había ningún techo de tamaño). */
...
/* ── R2 sig V4 (idéntico a upload-wallpaper) ────────────────────────── */
interface R2Config {
    accountId: string
    accessKeyId: string
...
Deno.serve(async (req: Request) => {
    ...
    // El `image_mime` del cliente se ignora a propósito: manda la firma real.
    ...
    try {
        // key con cache-buster (uuid) por subida → la URL nueva invalida la vieja.
        const key = `Avatares/${safeId(_g.userId!)}/${crypto.randomUUID()}.${_v.ext}`
