// admin/supabase/functions/upload-vtli-draft-cover/index.ts v1.3
// v1.3 — AUDITORÍA PARTE 4 — valida magic bytes + tope de tamaño con
//        _shared/upload.ts.
// v1.2 — GENÉRICO: acepta target "draft" (Estudio Manual, default) o "carousel"
//        (Zak'Haar Posts). Sube la portada a R2 y la guarda en
//        vtli_drafts.cover_image_url o zakhaar_carousels.cover_image_url según
//        target. Compatible hacia atrás: sin target → "draft" como antes.
// v1.1 — Ola C #3 Fase 3: token de Clerk requerido; sin fallback admin_clerk_id.
// Atelier — sube una IMAGEN DE PORTADA para un storyboard del Estudio Manual o
// para un carrusel de Zak'Haar Posts. La imagen va a Cloudflare R2; la DB solo
// guarda la URL en la columna cover_image_url de la tabla correspondiente.
//
//   POST { admin_clerk_id, target?: "draft"|"carousel", draft_id?, carousel_id?,
//          image_base64 (sin prefijo data:), image_mime }
//   →    { success, cover_image_url }
//
// Deploy:
//   cd "/Users/diego/Documents/Red Solar Viva/admin"
//   supabase functions deploy upload-vtli-draft-cover --no-verify-jwt
//
...
async function patchCover(
    ...
    await fetch(`${cfg.url}/rest/v1/${table}?id=eq.${recordId}`, {
        method: "PATCH",
        headers: {
...
Deno.serve(async (req: Request) => {
    ...
    let adminClerkId = ""
    const target = body?.target === "carousel" ? "carousel" : "draft"
    const coverTable =
        target === "carousel" ? "zakhaar_carousels" : "vtli_drafts"
    const recordId =
        target === "carousel"
            ? String(body?.carousel_id ?? "").trim()
            : String(body?.draft_id ?? "").trim()

    if (!recordId) return jsonResponse(400, { error: "missing_record_id" })
    ...
    let coverUrl: string
    try {
        const key = `Veo tu Luz Interna/Imagenes/Atelier/Covers/${todayDateString()}/${crypto.randomUUID()}.${_v.ext}`
        coverUrl = await uploadToR2(r2, _v.bytes, key, _v.mime)
    } catch (err: any) {
    ...
    await patchCover(supabase, coverTable, recordId, coverUrl)

    return jsonResponse(200, { success: true, cover_image_url: coverUrl })
})
