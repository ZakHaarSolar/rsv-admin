-- Red Solar Viva · COMUNIDAD · Reveal por contacto (Tripulante oculto -> su interlocutor)
-- =====================================================================
-- Aplicar: Supabase Dashboard -> SQL Editor -> New Query -> Run.
--
-- MODELO 1 (confirmado por Zak): un Tripulante OCULTO (visible=false) puede
-- navegar el directorio Y contactar a Tripulantes visibles. Al iniciar una
-- conversacion, su tarjeta se revela UNICAMENTE a la persona con quien habla
-- (en el chat), nunca al directorio publico.
--
-- Antes: get_community_profile exigia cp.visible -> el card del remitente
-- oculto salia 'not_found' en el chat de su interlocutor. Ahora: ademas de
-- visible, se devuelve el card si existe una conversacion (dm_conversations)
-- entre el viewer y el target. Los guards de bloqueo/baneo (_community_blocked
-- / _community_banned) y el alias no-vacio se conservan intactos.
--
-- Reproduce el cuerpo VIGENTE (20260622_community_photo.sql: bloqueos/baneos +
-- scan + edad + medallas + foto opt-in) y cambia SOLO el WHERE de visibilidad.
-- Misma firma (text, text) -> CREATE OR REPLACE, sin DROP. Sin cambios de
-- cliente: el chat ya hace fallback gracioso ante not_found y ahora recibe el
-- card real.
