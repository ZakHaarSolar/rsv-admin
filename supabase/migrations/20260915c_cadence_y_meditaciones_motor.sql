-- Red Solar Viva · cadence semanal/mensual + meditaciones en el Motor
-- =====================================================================
-- Aplicar: Supabase Dashboard → SQL Editor → New Query → Run.
--
-- (1) get_tripulante_extras v11: subscription_cadence
--     'cortesia' | 'semanal' | 'mensual' | NULL
--     Semanal = ciclo de 10 días o menos; gift_* = cortesía.
-- (2) admin_get_user_meditaciones_owned: lista canjes/compras de
--     meditaciones de un nodo (espejo de admin_get_user_codices_full).

DROP FUNCTION IF EXISTS public.get_tripulante_extras(TEXT, TEXT);

CREATE OR REPLACE FUNCTION public.get_tripulante_extras(
...
BEGIN
    ...
    active_sub AS (
        SELECT
            ...
            CASE
                WHEN m.stripe_subscription_id LIKE 'gift_%' THEN 'cortesia'
                WHEN m.current_period_start IS NOT NULL
                 AND m.current_period_end IS NOT NULL
                 AND (m.current_period_end - m.current_period_start)
                     <= INTERVAL '10 days'
                    THEN 'semanal'
                WHEN m.group_name IN ('sintonia', 'cuasar', 'inmersion', 'pulsar')
                    THEN 'mensual'
                ELSE NULL
            END AS cadence
        FROM matching m
    ...
    SELECT
        ...
        (SELECT cadence FROM active_sub) AS subscription_cadence;
END;
$$;
...
CREATE OR REPLACE FUNCTION public.admin_get_user_meditaciones_owned(
...
BEGIN
    ...
    RETURN QUERY
    SELECT
        mo.meditacion_id::text,
        replace(mo.meditacion_id, '-', ' ')::text AS title,
        mo.acquired_via::text,
        mo.acquired_at
    FROM public.meditaciones_owned mo
    WHERE mo.clerk_user_id = p_target_clerk_id
    ORDER BY mo.acquired_at DESC NULLS LAST;
END;
$$;

REVOKE EXECUTE ON FUNCTION public.admin_get_user_meditaciones_owned(TEXT, TEXT)
    FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.admin_get_user_meditaciones_owned(TEXT, TEXT)
    TO service_role;
